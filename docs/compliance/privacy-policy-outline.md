...
| Coarse home location | `User.homeZip`, `User.homeLat`, `User.homeLng` | Yes (optional) | **User-entered, not device GPS, in the MVP.** Two uses: (1) route-planner distance math, computed in `lib/routePlanner.ts` and scored/ranked by `lib/route.ts`; (2) alert targeting — `pickNearbyRecipients` (`lib/alerts.ts`) compares every user's home coordinates against a new report's store to pick alert recipients (`app/api/reports/route.ts`). Never displayed to other users (`docs/compliance.md`). Nullable — the account works without it. |
